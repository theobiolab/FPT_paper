...
* It has been modified from the original version to increase code transparency and efficiency.
*
*/
...
class scoring2D_v3 {
    ...
    public: 
        ...
        // declaration of the set_laplacian_TF function to be defined in the file that defines the child class
        virtual void set_laplacian_TF(IOType TF_conc)=0; 

        //void set_laplacian_red_class(Matrix<IOType,Dynamic,1> pset, IOType TF_conc){
        //     set_laplacian_red<T,IOType>(pset, TF_conc, this->laplacian, this->laplacian_TF); 
        //}
        ...
        virtual IOType compute_response(Matrix<IOType,Dynamic,1> ss_vector)=0; //previously compute_response_class. virtual function so it must be defined in the child class

        
        IOType set_conc_and_compute_response(IOType TF_conc){ //previously Response_class
            set_laplacian_TF(TF_conc); 
            Matrix<IOType,Dynamic,1> ss_vector = get_steady_state(true); 
            return compute_response(ss_vector); 
        }

        Matrix<T, Dynamic, Dynamic> &chooselap(Matrix<T,Dynamic,Dynamic> &laplacian, Matrix<T,Dynamic,Dynamic> &laplacian_TF, bool &use_laplacian_TF){
        //based on this:https://stackoverflow.com/questions/54503278/c-assigning-reference-variable-in-an-if-statement
            if (use_laplacian_TF){
                return laplacian_TF;
            }else{
                return laplacian;
        ...
        Matrix<IOType,2,1> SimpleScore(Matrix<IOType,2,1> TF_range, IOType n_points,
                                        IOType low_acc, IOType up_acc, bool computemeanscore = true, bool computecvscore = true, IOType sqpar=0.01){
            ...
            set_laplacian_TF(pow(10, low)); 
            Matrix<IOType,2,1> InitialPoint = get_Mean_CV_FPT(true, true );  //RMC this could be changed to (true, false) so that only mean is computed at first, so that if the point is to be discarded the cv is not computed
            ...
            int accepted = 0;
            IOType std_sq = sqpar * sqpar; //RMC adding option to change this
            IOType bignum = 100000000;
            IOType tmp_mean_scores =bignum;
            IOType tmp_cv_scores = bignum;
            IOType mean_score = bignum;    
            IOType cv_score = bignum;
            Matrix<IOType,2,1> new_point;
            for (int i = 1; i<n_points; i++){
                 //RMC commenting out this bit, it is redundant with what comes afterwards
                // initialize the variables mean_score and cv_score
                //if (i == 1 || accepted == 0){                
                //    set_laplacian_TF(pow(10, TF_conc));
                //    
                //    try{ 
//
...
//
                ...
                try{ 
                    ...
                    if (computemeanscore){ 
                            ...
                            tmp_mean_scores= bignum;
                        }
                    if (computecvscore) {  
                            tmp_cv_scores  = this->gaussian_score(new_point(1), InitialPoint(1), std_sq);
                        }else{
                            tmp_cv_scores=bignum;

                    }
                        
                   // keep only the lowest mean score
        ...
        std::vector<IOType> trim_range_response(Matrix<IOType,Dynamic,1>TF_range,
            ...
            IOType conc = low;
            IOType min_resp = set_conc_and_compute_response(pow(10, conc)); 
            IOType max_resp = min_resp; //no need to recompute it again
            conc_array[0]=conc;
            resp[0]=min_resp;
            for (int i=1; i<n_points; i++){
                conc += delta; 
                conc_array[i] = conc; 
                resp[i] = set_conc_and_compute_response(pow(10, conc)); 
        ...
        Matrix<IOType,2,1> redef_score_RESP(const int n_points, const IOType deltaTH, 
                                            const IOType low_acc, const IOType up_acc, bool computemeanscore = true, bool computecvscore = true, IOType sqpar=0.01){
            ...
            if (trimmed_range[2] >= deltaTH){
                ...
                return SimpleScore(new_TF_range,15,low_acc,up_acc, computemeanscore, computecvscore,sqpar);
            } else {
        ...
        Matrix<IOType,2,1> redef_score_noRESP(Matrix<IOType,Dynamic,1>TF_range_noresp, 
            ...
            if (trimmed_range[2] < deltaTH){
                return SimpleScore(TF_range_noresp,15,low_acc,up_acc, computemeanscore, computecvscore,sqpar);
            } else {
                Matrix<IOType,2,1> ret;
                ret << -1.0, -1.0; 
                return ret;
